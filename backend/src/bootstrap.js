...
export function bootstrap(config) {
  ...
  const dejaInitialise = db.prepare("SELECT 1 FROM meta WHERE key = 'catalogue_initialise'").get();
  if (!dejaInitialise) {
    const produits = createProduitsRepo(db);
    const now = new Date().toISOString();
    transaction(db, () => {
      for (const p of PRODUITS_INITIAUX) produits.create(p, now);
      db.prepare("INSERT INTO meta (key, value) VALUES ('catalogue_initialise', ?)").run(now);
    })();
  }

  const utilisateurs = createUtilisateursRepo(db);
  if (utilisateurs.count() === 0) {
    utilisateurs.create(config.adminEmail, bcrypt.hashSync(config.adminPassword, 12));
  }
